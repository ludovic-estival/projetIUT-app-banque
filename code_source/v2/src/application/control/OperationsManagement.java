...
public class OperationsManagement {
	...
	public Operation enregistrerVirement() {
		...
		if (op != null) {
			try {
				AccessOperation ao = new AccessOperation();
				int idCompteDestination = 0;
				try {
					idCompteDestination = Integer.parseInt(op.idTypeOp.trim().substring(0, 5));
				} catch (NumberFormatException ex) {
					ex.printStackTrace();
				}
				ao.insertVirement(this.compteConcerne.idNumCompte, idCompteDestination, op.montant);
	...
	public void gererPrelevements() {
		PrelevementsManagement em = new PrelevementsManagement(this.primaryStage, this.dbs, clientDuCompte, compteConcerne);
		em.doPrelevementsManagementDialog();
	}
}